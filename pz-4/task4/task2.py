...
def content_to_list(content, currency):
    ...
    for i in range(0, len(content_list)):
        ...
        if content_list[i] == '/Valute':
            finish = i
            foo = get_currency_rate(content_list[start + 1:finish], currency) # проверяю является ли валюта необходимой пользователю, проверяя значения для каждой валюты, завёрнутые в теги valute
            if isinstance(foo, float):
                return foo
    return None


def get_currency_rate(some_list, user_cur): # получает срез, в котором находятся значения каждой валюты, постепенно перебирая все валюты - мы находим необходимуб
...
print(content_to_list(content, 'UsD'))
content_to_list(content, 'EUR')
